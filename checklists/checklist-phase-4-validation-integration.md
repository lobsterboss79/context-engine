...
- [ ] 1.1 [PLAN][DOC] Record the immutable/reference validation baseline: Phase 0–2 approved records, the Phase 3 closure record, clean closure commit `9862497`, approved configuration/fixture provenance, and any later approved baseline amendment with its authority and scope. Do not rewrite historical baseline evidence.
- [ ] 1.2 [PLAN][DOC] Define Phase 4 scope, exclusions, and the distinction among implementation testing, validation, proving, and production readiness. State that implementation-level tests are evidence inputs, not a substitute for Phase 4 integrated validation or proving.
- [ ] 1.3 [PLAN][DOC] Establish evidence-preservation rules: immutable original result artifacts; controlled copies/derivations with lineage; task/request, fixture/Source, configuration/Bootstrap, environment, version/commit, command/procedure, raw result, rendering, reviewer assessment, finding, and disposition linkage as applicable. Preserve failures and inconclusive results as evidence.
- [ ] 1.4 [PLAN][DOC] Establish expected-result controls before execution: predetermined expected outcomes and acceptance/negative criteria, known limitations, fixture identity, reviewer basis, and rules for recording an unanticipated result. Expected behavior may not be changed merely to make a test pass.
- [ ] 1.5 [PLAN][DOC] Define validation result states: **PASS**, **FAIL**, and **INDETERMINATE** where evidence is incomplete, contradictory, not reproducible, or insufficient to evaluate the established result. INDETERMINATE is not PASS and cannot be silently treated as an absence of a finding.
- [ ] 1.6 [PLAN][DOC] Establish finding severities: **BLOCKER** (prevents safe/valid continuation or invalidates a material claim); **MATERIAL** (materially affects approved semantics, governance/security, validation validity, scope, or a claimed capability); **MINOR** (bounded non-material defect/inconsistency requiring governed disposition); and **OBSERVATION** (non-blocking evidence/lesson that does not alter approved behavior). Establish a finding register with unique identity, evidence links, owner, status, classification, disposition authority, and closure evidence.
- [ ] 1.7 [PLAN][DOC] Establish finding types: **implementation**, **integration**, **security/governance**, **operational**, **design**, **proving-protocol**, and **Consumer-usability**. Classify severity and type independently; an implementation defect is not automatically a design deficiency, and vice versa.
- [ ] 1.8 [PLAN][DOC] Define finding ownership and disposition: record cause analysis separately from conclusion; preserve original evidence; identify whether a finding requires no change, authorized remediation, design/governance review, protocol discard/restart, or a later-phase candidate. Findings close only with evidence.
- [ ] 1.9 [PLAN][H3] Define H3 escalation boundaries and Codex remediation boundaries. Stop for material or genuinely ambiguous scope, architecture, technology, security, governance, semantic, data-model, dependency, Consumer-protocol, or phase decision. Require Project Owner approval before material remediation; no remediation authority arises from finding discovery alone.
- [ ] 1.10 [PLAN][DOC] Define remediation evidence and retest requirements: causal hypothesis, authorized change boundary, exact changed baseline, focused retest, affected-suite retest, relevant end-to-end regression validation, residual limitations, and evidence-based closure.
- [ ] 1.11 [PLAN][DOC] Establish anti-overfitting controls. A remedy must address a general mechanism/invariant, not hard-code a proving case, project vocabulary, Consumer identity, expected answer, or special path absent an approved general requirement. Expected results may not be altered solely to eliminate failure.
- [ ] 1.12 [PLAN][DOC] Establish proving evidence preservation and independent evaluation expectations: freeze protocol/package/rendering before Consumer evaluation; retain immutable original Consumer-facing evidence and interventions; separate construction from evaluation; preserve omissions, inappropriate inclusions, qualifications, and negative outcomes; and obtain evaluation by a reviewer not responsible for changing the result under review where practicable.
...
- [ ] 1.14 [PLAN][DOC] Define reserved-Consumer protection and proving-run discard criteria. No planned reservation may expose Project facts, package contents, task-specific material state, or other contamination before actual preflight. A run is discarded/restarted for freshness FAIL/INDETERMINATE, protocol breach, material uncontrolled intervention, unpreserved original evidence, or another condition that invalidates the established evidence basis.
- [ ] 1.15 [PLAN][H3] Record the TD-14 stop/reopening procedure. If evidence shows relevant, authorized, in-scope information necessary to Required Context or a success criterion is materially or repeatedly undiscoverable through approved deterministic mechanisms, causing incorrect/insufficient context, material information loss, or inability to continue meaningfully: stop affected work; preserve request/fixture/scope/result evidence; record a MATERIAL finding; trigger H3; identify potential TD-14 reopening; and await governance. Do not introduce AI or vector technology as a workaround.
- [ ] 1.16 [PLAN][H3] Define the material Phase 4 scope-change procedure: identify the proposed change, evidence and impact, affected requirements/architecture/semantics/technology, alternatives, and required Project Owner decision before proceeding. A useful future capability is not authorization.
- [ ] 1.17 [PLAN][DOC] Assemble the Phase 4 validation-governance package: baseline register, scope/exclusions, methodology, expected-result register, evidence inventory/template, finding register/template, remediation/retest protocol, contamination/proving-protocol architecture, TD-14 control, H3 escalation record, and traceability to this checklist.
